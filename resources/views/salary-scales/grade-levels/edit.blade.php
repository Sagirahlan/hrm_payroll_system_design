...
<div class="container mt-4">
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">Edit Grade Level for {{ $salaryScale->acronym }} - {{ $salaryScale->full_name }}</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('salary-scales.grade-levels.update', [$salaryScale->id, $gradeLevel->id]) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="basic_salary" id="basic_salary" step="0.01" class="form-control" value="{{ old('basic_salary', $gradeLevel->basic_salary) }}" required min="0">
                        @error('basic_salary')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="grade_level" id="grade_level" class="form-control" value="{{ old('grade_level', $gradeLevel->grade_level) }}" required min="1">
                        @error('grade_level')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="step_level" id="step_level" class="form-control" value="{{ old('step_level', $gradeLevel->step_level) }}" required min="1">
                        @error('step_level')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
